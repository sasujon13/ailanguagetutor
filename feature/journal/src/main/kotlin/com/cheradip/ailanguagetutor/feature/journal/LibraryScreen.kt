...
class LibraryViewModel @Inject constructor(
    ...
    val activities = searchQuery.flatMapLatest { q ->
...
fun LibraryScreen(
    ...
    CheradipScrollScreen(
        ...
        if (activities.isNotEmpty()) {
            item { SectionHeader(title = "Recent activities") }
        ...
        item { SectionHeader(title = "Documents") }
        if (documents.isEmpty()) {
            item {
                EmptyStateHint("No documents yet. Scan or import from Home.", icon = Icons.Default.Description)
            }
        } else {
            items(documents.size, key = { documents[it].id }) { index ->
                DocumentCard(doc = documents[index], onClick = { onOpenDocument(documents[index].id) })
            }
        }
    }
}
