# Ensures cloud-api .venv exists with a working pip, then installs the package editable.
param(
    [switch]$Quiet
)
...
function New-CloudApiVenv {
    if (Test-Path ".venv") {
        Write-Host "Removing broken cloud-api .venv..."
        Remove-Item -Recurse -Force ".venv"
    }
    Write-Host "Creating cloud-api virtual environment..."
    python -m venv .venv
    if ($LASTEXITCODE -ne 0) {
